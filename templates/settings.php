          ...
          <div>
            <ul class="flex flex-wrap -mb-px" role="tablist">
              <li role="presentation" class="mr-2">
                <button class="inline-block py-2 px-4 text-sm font-medium text-indigo-600 bg-white rounded-t-lg border-b-2 border-indigo-600" data-tab-target="#tab-general" type="button">General</button>
              </li>
              <li role="presentation" class="mr-2">
...
?>
                      ...
                      <div class="mt-2">
                        <div class="breaks-list" data-day-index="<?php echo $i; ?>">
                          <?php
  if (!empty($settings['availability'][$i]['breaks']) && is_array($settings['availability'][$i]['breaks'])):
    foreach ($settings['availability'][$i]['breaks'] as $idx => $break):
?>
                                  <div class="break-row" style="display:flex;gap:.5rem;align-items:center;margin-bottom:.5rem">
                                      <input type="time" name="booking_app_settings[availability][<?php echo $i; ?>][breaks][<?php echo $idx; ?>][start]" value="<?php echo esc_attr($break['start']); ?>" class="flow-input--time" data-name-start />
                                      <input type="time" name="booking_app_settings[availability][<?php echo $i; ?>][breaks][<?php echo $idx; ?>][end]" value="<?php echo esc_attr($break['end']); ?>" class="flow-input--time" data-name-end />
