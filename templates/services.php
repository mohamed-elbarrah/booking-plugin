            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><?php esc_html_e('Status', 'mbs-booking'); ?></th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider"><?php esc_html_e('Actions', 'mbs-booking'); ?></th>
                    </tr>
                </thead>
                    ...
                    <?php if (empty($services)): ?>
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-sm text-gray-500">
                                <?php esc_html_e('No services created yet.', 'mbs-booking'); ?>
                            </td>
                        </tr>
                    <?php
else: ?>
                        <?php foreach ($services as $service): ?>
                            <tr data-service-id="<?php echo esc_attr($service->id); ?>" class="draggable-row" draggable="true">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <?php echo esc_html($service->duration); ?> <?php esc_html_e('min', 'mbs-booking'); ?>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <?php if (floatval($service->price) <= 0): ?>
                                        <span class="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded border border-green-400"><?php esc_html_e('Free', 'mbs-booking'); ?></span>
                                    <?php
        else: ?>
        ...
        <div class="relative w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="px-6 py-6 lg:px-8">
                    <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white"><?php esc_html_e('Add New Service', 'mbs-booking'); ?></h3>
